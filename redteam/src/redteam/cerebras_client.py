...
class CerebrasClient:
    ...
    def _schema(model: Type[T]) -> dict[str, object]:
        ...
        assert issubclass(model, BaseModel), "must be a Pydantic model subclass"
        js = model.model_json_schema()
        
        # Iteratively set additionalProperties: False on all objects (NASA Rule 1: no recursion)
        stack = [js]
        for _ in range(1000):  # Bounded loop (NASA Rule 2)
            if not stack:
                break
            curr = stack.pop()
            if isinstance(curr, dict):
                if curr.get("type") == "object":
                    ...
                    if "properties" in curr:
                        curr["required"] = list(curr["properties"].keys())
                    ...
                    if isinstance(val, dict):
                        ...
                        for item in val:
                            if isinstance(item, dict):
                                stack.append(item)
        assert not stack, "schema stack not fully processed"
