...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--server":
        import uvicorn
        uvicorn.run("memeforge.web:app", host="0.0.0.0", port=8000, reload=True)
    elif len(sys.argv) == 1:
        print("No image path provided. Starting web server on http://localhost:8000 ...")
        import uvicorn
        uvicorn.run("memeforge.web:app", host="0.0.0.0", port=8000)
    else:
